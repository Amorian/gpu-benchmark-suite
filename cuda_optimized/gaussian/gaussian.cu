  ...
  printf("WG size of kernel 1 = %d, WG size of kernel 2= %d X %d\n", MAXBLOCKSIZE, BLOCK_SIZE_XY, BLOCK_SIZE_XY);
    ...
    for(i=1;i<argc;i++) {
      if (argv[i][0]=='-') {// flag
        ...
        cudaHostAlloc((void **) &a, Size * Size * sizeof(float), cudaHostAllocWriteCombined);
        ...
        cudaHostAlloc((void **) &m, Size * Size * sizeof(float), cudaHostAllocWriteCombined);
              break;
            case 'f': // platform
              i++;
...
void ForwardSub()
{
  ...
  cudaStreamCreate(&stream3);
	...
	// copy memory to GPU
	cudaMemcpyAsync(m_cuda, m, Size * Size * sizeof(float),cudaMemcpyHostToDevice, stream1);
	cudaMemcpyAsync(a_cuda, a, Size * Size * sizeof(float),cudaMemcpyHostToDevice, stream2);
	cudaMemcpyAsync(b_cuda, b, Size * sizeof(float),cudaMemcpyHostToDevice, stream3);
  ...
  cudaDeviceSynchronize();
	...
	cudaMemcpyAsync(m, m_cuda, Size * Size * sizeof(float),cudaMemcpyDeviceToHost, stream1);
	cudaMemcpyAsync(a, a_cuda, Size * Size * sizeof(float),cudaMemcpyDeviceToHost, stream2);
	cudaMemcpyAsync(b, b_cuda, Size * sizeof(float),cudaMemcpyDeviceToHost, stream3);
  cudaDeviceSynchronize();
	cudaFree(m_cuda);
	cudaFree(a_cuda);
	cudaFree(b_cuda);
