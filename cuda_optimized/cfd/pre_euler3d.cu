...
int main(int argc, char** argv)
{
    ...
    float* h_areas;
		...
		float* h_normals;
    cudaHostAlloc((void **) &h_areas, nelr * sizeof(float), cudaHostAllocWriteCombined);
    cudaHostAlloc((void **) &h_elements_surrounding_elements, nelr * NNB * sizeof(int), cudaHostAllocWriteCombined);
    cudaHostAlloc((void **) &h_normals, nelr * NDIM * NNB * sizeof(float), cudaHostAllocWriteCombined);
    ...
    cudaStreamCreate(&stream3);
		...
		upload<float>(areas, h_areas, nelr, stream1);
		...
		upload<float>(normals, h_normals, nelr*NDIM*NNB, stream3);
	...
	for(int i = 0; i < iterations; i++)
  {
    ...
    copy<float>(old_variables, variables, nelr*NVAR, streams[i]);
