...
int main(int argc, char** argv)
{
	...
	double* normals;
	{
		...
		cudaHostAlloc((void **) &h_areas, nelr * sizeof(double), cudaHostAllocWriteCombined);
    cudaHostAlloc((void **) &h_elements_surrounding_elements, nelr * NNB * sizeof(int), cudaHostAllocWriteCombined);
    cudaHostAlloc((void **) &h_normals, nelr * NDIM * NNB * sizeof(double), cudaHostAllocWriteCombined);
    ...
    cudaStreamCreate(&stream3);
		...
		upload<double>(areas, h_areas, nelr, stream1);

		elements_surrounding_elements = alloc<int>(nelr*NNB);
		upload<int>(elements_surrounding_elements, h_elements_surrounding_elements, nelr*NNB, stream2);

		normals = alloc<double>(nelr*NDIM*NNB);
		upload<double>(normals, h_normals, nelr*NDIM*NNB, stream3);

		cudaDeviceSynchronize();

		cudaFreeHost(h_areas);
	  cudaFreeHost(h_elements_surrounding_elements);
	  cudaFreeHost(h_normals);
	...
	cudaStream_t* streams = new cudaStream_t[iterations];

	// Begin iterations
	for(int i = 0; i < iterations; i++)
	{
		cudaStreamCreate(&streams[i]);
		copy<double>(old_variables, variables, nelr*NVAR, streams[i]);
